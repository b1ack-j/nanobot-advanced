...
Every requirements document must start with a revision header. On first creation, use revision 1. On updates (loop-back or new pass), increment the revision.
...
- Do NOT use version suffixes in filenames (no `requirements-v2.md`) -- use revision headers
