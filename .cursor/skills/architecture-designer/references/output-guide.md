...
Every architecture document must start with a revision header. On first creation, use revision 1. On updates (loop-back from review or new pass), increment the revision.
...
When architectural decisions imply concrete technical artifacts, also create or update files in `docs/<project>/specs/`:

- **data-models.md**: DB schemas, entity definitions, migration SQL — when the architecture defines new tables or data structures
- **api-contracts.md**: endpoint signatures, request/response shapes — when the architecture defines new API surfaces
- **protocols.md**: WebSocket messages, push payloads, event formats — when the architecture defines new communication protocols

These specs are the implementor-facing extraction of architectural decisions. Keep them in sync with the architecture document.
