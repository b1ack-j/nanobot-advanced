...
Every review document must start with a revision header. On first creation, use revision 1. On updates (new pass or re-review after loop-back), increment the revision.
...
2. Update the date
3. Clearly indicate which findings are new vs carried forward from previous revisions
4. Return a **change summary** to the orchestrator (1-3 sentences describing what changed and why) for logging in CHANGELOG.md
...
- Do NOT use version suffixes in filenames (no `review-v2.md`) -- use revision headers
